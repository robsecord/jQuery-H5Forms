...
// Global Function for Validating Email Addresses; Used by H5Forms to validate email addresses
var customEmailValidation;
...
(function($, undefined) {
    ...
    var defaults = {
        ...
        validation: {
            ...
            customPatterns: [
                ...
                {
                    name:    'email', 
                    pattern: customEmailValidation, 
                    message: 'Your email adress is invalid, please provide a valid email address.'
                }
            ]
        },
    ...
    $h5f.fn.extend({
        ...
        validatePattern: function(fieldEl, patternData, validationType) {
            // Build Array of Associated Elements; Adds Current Element First
            var _self = this, isFieldValid = true, assocEls = this.getAssociatedElements(fieldEl, true);

            // Get Pattern to Test Against
            var pattern = patternData.pattern, errorMsg = patternData.message;
            
            // Private Function for Validating against Pattern
            var testPattern = function(testValue, usePattern) {
                if (typeof usePattern == 'function') {
                    usePattern.apply(_self, [testValue]);
                } else {
                    return testValue.match(usePattern);
                }
            };
            ...
            switch( fieldEl[0].nodeName.toLowerCase() ) {
                case 'select':
                    for( i = 0; i < assocEls.length; i++ ) {
                        v = assocEls[i][0].options[assocEls[i][0].selectedIndex].value.trim();
                        if (!testPattern(v, pattern)) { isFieldValid = false; break; }
                    }
                    break;
                ...
                case 'textarea':
                    ...
                    if( rel == 'text-multi' ) {
                        ...
                        // Validate against min/max attributes
    ...
    /**
     * Custom Email Validation Function; used by H5Forms to Test Email Addresses
     *
     * @method customEmailValidation
     * @param testValue {String} An email address to test the validity of
     * @return boolean
     */
    customEmailValidation = function(testValue) {
        // Taken from: Validate an E-Mail Address with PHP, the Right Way
		// http://www.linuxjournal.com/article/9585
        var isValid = true,
            atIndex = testValue.lastIndexOf('@');
        if (atIndex == -1) {
            isValid = false;
        } else {
            var domain    = testValue.substring(atIndex+1),
                local     = testValue.substring(0, atIndex),
            ...
            else if (domain.lastIndexOf('.') == -1) {
                // invalid domain part
                isValid = false;
            }
            else if (domain.charAt(0) == '.' || domain.charAt(domainLen-1) == '.') {
